...
def rgb2gray(img_0):
    if len(img_0.shape) == 1:
        ...
        img_gray = color.rgb2gray(img_0) * 255  # Gray = R*0.2989 + G*0.587 + B*0.114
    else:
        raise ValueError("ValueError: Number of image channels not supported!")
    img_gray = img_gray.astype(np.uint8)
    output_path = Path(root_path) / "results" / "img_gray" / f"img_gray_{Path(img_path).stem}.jpg"
...
def remove_reflections(img_gray, kernel_size=3, iterations=1, inpaintRadius=3, flags=cv2.INPAINT_NS):
    ...
    if len(valid_indices) > 0:
        ...
        mask_path = Path(root_path) / "results" / "binary_mask" / f"binary_mask_{Path(img_path).stem}.jpg"
    ...
    repaired_path = Path(root_path) / "results" / "img_repaired" / f"img_repaired_{Path(img_path).stem}.jpg"
...
def histogram_stretching(img_gray):
    ...
    output_path = Path(root_path) / "results" / "img_256" / f"img_256_{Path(img_path).stem}.jpg"
    save_dir = output_path.parent
    save_dir.mkdir(parents=True, exist_ok=True)
...
def compress_grayscale(img_256, levels):
    global img_comp
    ...
    if levels == 256:
        ...
        scale = 256 // levels
        img_comp = (img_256 // scale) * scale
        output_path = Path(root_path) / "results" / "img_comp" / f"img_comp_{levels}_{Path(img_path).stem}.jpg"
        save_dir = output_path.parent
        save_dir.mkdir(parents=True, exist_ok=True)
...
def glcm_texture_features(img_comp, properties, distances, angles):
...
def save_glcm_matrix(glcm_matrix):
    angle_dic = {0: "0°", 1: "45°", 2: "90°", 3: "135°"}
    for i in range(0, glcm_matrix.shape[2]):
        for j in range(0, glcm_matrix.shape[3]):
            glcm_path = Path(root_path) / "results" / "GLCM_matrix" / f"GLCM_matrix_{Path(img_path).stem}_{i}_{j}.jpg"
...
def visualize_features(features, distances):
    ...
    xlsx_path = Path(root_path) / "results" / "data" / f"data_{Path(img_path).stem}.xlsx"
    save_dir = xlsx_path.parent
    save_dir.mkdir(parents=True, exist_ok=True)
    wb.save(xlsx_path)
    print("GLCM features saved successfully!")
    print("保存路径：", xlsx_path)
...
if __name__ == "__main__":
    ...
    angles = [0, np.pi / 4, np.pi / 2, 3 * np.pi / 4]  # 所有4个方向
